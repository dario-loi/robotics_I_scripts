...
def test_direct():
    ...
    assert rot.direct_rot_mat(np.pi, [1, 0, 0]).all() == np.array([[1, 0, 0], [0, -1, 0], [0, 0, -1]]).all()
    assert rot.direct_rot_mat(np.pi / 2, [1, 0, 0]).all() == np.array([[1, 0, 0], [0, 0, -1], [0, 1, 0]]).all()
...
def test_inverse():
    ...
    assert rot.inverse_rot_mat(np.array([[1, 0, 0], [0, -1, 0], [0, 0, -1]]))[0][1] == np.pi
