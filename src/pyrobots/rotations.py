...
def direct_rot_mat(theta: float, axis: np.ndarray) -> np.ndarray:
    ...
    c = np.cos(theta)
    s = np.sin(theta)
    rot_mat = np.array([[c + ux**2 * (1 - c), ux * uy * (1 - c) - uz * s, ux * uz * (1 - c) + uy * s],
                        [uy * ux * (1 - c) + uz * s, c + uy**2 * (1 - c), uy * uz * (1 - c) - ux * s],
                        [uz * ux * (1 - c) - uy * s, uz * uy * (1 - c) + ux * s, c + uz**2 * (1 - c)]])
    return rot_mat

def inverse_rot_mat(R: np.ndarray) -> Tuple[Union[float, Tuple[float, float]], Optional[np.ndarray]]:
    """ 
    ...
    s = 1/2 * np.sqrt((R[2, 1] - R[1, 2])**2 + (R[0, 2] - R[2, 0])**2 + (R[1, 0] - R[0, 1])**2)
    ...
    if np.isclose(theta, 0): # undefined axis
        ...
        if np.isclose(r[0] * r[1], R[0, 1] / 2):
            r[2] = R[0, 2] / 2 / r[0]
        elif np.isclose(r[0] * r[2], R[0, 2] / 2):
            r[1] = R[0, 1] / 2 / r[0]
        elif np.isclose(r[1] * r[2], R[1, 2] / 2):
            r[0] = R[0, 1] / 2 / r[1]
        else:
            raise ValueError("Could not resolve sign ambiguity.")
...
if __name__ == "__main__":
    inverse_rot_mat(np.array([[ 0.70710678, -0.70710678,  0.        ],
